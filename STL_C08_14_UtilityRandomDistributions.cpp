...
void print_distro(T distro, size_t samples) {
    ...
    for(size_t i = 0; i < samples; i++) { m[distro(e)]++; }
    ...
    for(const auto [randval, count] : m) {
        ...
        cout << setw(3) << randval << " : " << string(count / max_div, '*') << '\n';
    }
}
...
int main(int argc, char** argv)
{
    ...
    initializer_list<double> weight_rates {0, 1, 1, 0};
    cout << "piecewise_linear_distribution\n";
    print_distro(piecewise_linear_distribution<double>{begin(intervals), end(intervals), begin(weight_rates)}, samples);
